...
def load_csv(path: str) -> pd.DataFrame:
    ...
    try:
        # First try: parse common date columns if present
        df = pd.read_csv(path)
...
def infer_columns(df: pd.DataFrame):
    ...
    for c in cols:
        ...
        if c in ("date", "order_date", "sale_date", "timestamp"):
...
def sales_by_region(df: pd.DataFrame, sales_col: str, region_col: str):
...
def monthly_sales_trend(df: pd.DataFrame, sales_col: str, date_col: str):
    """Aggregate sales by month (requires parsed datetime column)."""
...
def plot_line(series: pd.Series, title: str, ylabel: str, filepath: str, figsize=(10, 6)):
    plt.figure(figsize=figsize)
...
def run_analysis(csv_path: str, top_n: int = 10):
    ...
    if product_col:
        ...
        report_lines.append("Top products by sales:")
    ...
    if region_col:
        ...
        report_lines.append("Region column not detected; skipping region-level analysis.")
    ...
    if date_col and pd.api.types.is_datetime64_any_dtype(df[date_col]):
        ...
        plot_line(monthly, "Monthly Sales Trend", sales_col, PLOTS["time_series"])
    else:
    ...
    logging.info("Analysis complete. Outputs saved to %s", OUTPUT_DIR)


# ---- CLI ----
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Sales CSV Analysis (internship level)")
